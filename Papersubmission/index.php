...
if (isset($_POST['formsubmitted'])) {
    ...
    if (empty($_POST['college'])) {//if no name has been supplied 
        $error[] = 'Please Enter a College name ';//add to array "error"
    } else {
        $college = $_POST['college'];//else assign it a variable
    }
    if (empty($_POST['roll'])) {//if no name has been supplied 
    ...
    { // If everything's OK...
        ...
        if (mysql_num_rows($result_verify_email) == 0) { // IF no previous user is using this email .
            ...
            $query_insert_user = "INSERT INTO `members` ( `Username`, `Email`, `Password`, `Activation`, `Institute` , `Roll`,`Phone` ) VALUES ( '$name', '$Email', '$Password', '$activation' , '$college','$roll','$phone')";
