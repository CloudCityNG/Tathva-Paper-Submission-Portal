...
function add_marks()
{
    ...
    while ($get = mysqli_fetch_assoc($result))
    {
        ...
        echo '<tr><td>'.$i.'</td><td> '.$id1.' </td><td><a href='.$Link.' target=Viewer>'.$get["Upload"].'</a></td><td>Total Marks : '.$total.'</td></tr>';
        $i++;
        $total=0;
    }
    echo "</table>";
...
function Send_Mail()
{
    ...
    $two=$_SESSION['2_nm'];
    $three=$_SESSION['3_nm'];
    $message="<html><body><br><br><ol><li>$one</li><li>$two</li><li>$three</li></ol></body></html>";
    //mail($Email, 'Registration Confirmation', $message, 'From: server_norply@allstuffcodes.info');
    echo $message;
    $headers  = 'MIME-Version: 1.0' . "\r\n";
    $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
    $headers .= 'From: server_norply@allstuffcodes.info' . "\r\n";
    $result=mysqli_query($dbc,"select * from User where Permission = 0");
